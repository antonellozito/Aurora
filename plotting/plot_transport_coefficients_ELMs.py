...
def plot(namelist,exp_data,asim,out,rhop,D,D_ELM,v,v_ELM,rhop_transp,transp_coeffs,time_inter_ELM,time_intra_ELM,time_exp_shift=0.0,xmin=None,xmax=None,Dmin=None,Dmax=None,vmin=None,vmax=None):
    ...
    rhop_core_exp = exp_data[f'{namelist["imp"]}_density_plasma']['rhop'].transpose()[:,0]
    n_imp_core_exp = exp_data[f'{namelist["imp"]}_density_plasma'][f'n_{namelist["imp"]}'].transpose()
    
    rhop_edge_inter_ELM_exp = exp_data[f'{namelist["imp"]}_density_edge_inter_ELM']['rhop_data'].transpose()
    n_imp_inter_ELM_exp = exp_data[f'{namelist["imp"]}_density_edge_inter_ELM'][f'n_{namelist["imp"]}_data'].transpose()

    time_edge_ELM_cycle_exp = exp_data[f'{namelist["imp"]}_density_edge_ELM_cycle']['time'] + time_exp_shift
    rhop_edge_ELM_cycle_exp = exp_data[f'{namelist["imp"]}_density_edge_ELM_cycle']['rhop'].transpose()
    n_imp_ELM_cycle_exp = exp_data[f'{namelist["imp"]}_density_edge_ELM_cycle'][f'n_{namelist["imp"]}'].transpose()
    ...
    idx_intra_ELM_exp = np.argmin(np.abs(np.asarray(time_edge_ELM_cycle_exp) - time_intra_ELM))
    ...
    ax1.scatter(rhop_core_exp,n_imp_core_exp,color=blue,zorder=1,label='Experiment')
    for i in range(rhop_edge_inter_ELM_exp.shape[1]):
        ax1.scatter(rhop_edge_inter_ELM_exp[:,i],n_imp_inter_ELM_exp[:,i],color=blue,s=2,zorder=1)
    ...
    ax4.scatter(rhop_core_exp,n_imp_core_exp,color=blue,zorder=1,label='Experiment')
    ax4.scatter(rhop_edge_ELM_cycle_exp[:,idx_intra_ELM_exp],n_imp_ELM_cycle_exp[:,idx_intra_ELM_exp],color=blue,zorder=1)
